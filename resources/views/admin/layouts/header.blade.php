    ...
    <div class="container-fluid">
        ...
        <ul class="header-nav d-none d-md-flex">
            <li class="nav-item"><a class="nav-link"
                                    href="javascript:void(0)">{{(isset(auth()->user()->name) ? ucwords(auth()->user()->name) : '')}}</a>
            </li>
        </ul>
        ...
        <ul class="header-nav ms-3">
            <li class="nav-item dropdown">
                   ...
                   aria-expanded="false">
                    <div class="avatar avatar-md">
                        @if(!empty(auth()->user()->image))
                            <img class="avatar-img" src="{{route('admin.profile.get.image',auth()->user()->id)}}"
                                 alt="user@email.com">
                        @else
                            <img class="avatar-img" src="{{asset('admin/coreui')}}/assets/img/avatars/1.jpg"
                                 alt="user@email.com">
                        @endif
                    </div>
                </a>
                <div class="dropdown-menu dropdown-menu-end pt-0">
                    ...
                    <a href="javascript:void(0)" class="dropdown-item" data-coreui-toggle="modal"
                       data-coreui-target="#switch-modules" id="switch">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb my-0 ms-2">
                ...
                <li class="breadcrumb-item active">
                    <span> {{(!empty($p_title) && isset($p_title)) ? $p_title : ''}}</span></li>
            </ol>
        </nav>
    </div>
</header>
